...
if(model_type == "bmd"){
    ...
    dat = read.table("./R_code/BDM.txt",header=T)[,c("age", "spnbmd")]
    n = nrow(dat)
...
if(model_type == "APL"){
    ...
    dat = data.frame(read.csv("./r code/DIB4.csv"))
    #dat = read.csv("DIB4.csv")
    gender = dat[,"sex"]
    age = dat[,"v7age"]
    wgt = dat[,"v7weight"]
    hgt = dat[,"V7height"]
    str = dat[,"v7grip"]
    str = dat[,"v9grip"]
    apratio = dat[,"apratioV7"]
    apfat = dat[,"v7apfat"]
    
    sel_ind = intersect(which(is.na(wgt) == F), which(is.na(apratio) == F))
    n = length(sel_ind)
    p = 1
    
    wgt = wgt[sel_ind];apratio = apratio[sel_ind]
    X = as.matrix(wgt, ncol=1); Xt = as.matrix(wgt, ncol=1)
    y = as.matrix(apratio, ncol=1);yt = as.matrix(apratio, ncol=1)
}
