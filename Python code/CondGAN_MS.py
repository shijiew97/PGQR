...
#    sys.stdout.flush()
    ...
    def Loss_D(D_fake, D_real):
        out = torch.exp(D_real) - D_fake  #f-GAN
        #out = (D_real-1)**2 + D_fake**2 #least-square-GAN
        return out.mean()
      
    def Loss_G(D_fake):
        out = D_fake   #f-GAN
        #out = (D_fake-1)**2 #least-square-GAN
        return out.mean()
    ...
    for it in range(iteration):
        ...
        for h in range(K0):
            ...
            G.zero_grad()
            #Out_G1 = G(X0, Z, lam, batchnorm_on)
            ...
            if pen_on == 1:
              ...
              Out_G2 = G(X0, Z2, lam, batchnorm_on)
              dist = torch.abs(Out_G1 - Out_G2)
              #dist = torch.square(Out_G1 - Out_G2)
              pen = torch.mean( torch.log(  ( (  dist  / sd_y ) )*fac  + 1.0 / fac ) )
              #pen = torch.mean( 1.0 / (  ( (  dist  / sd_y ) )*fac ) )
              
              #pen = torch.log((torch.std(Out_G1)))
