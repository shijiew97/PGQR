...
def GGQ1_pen(y, X, Xt, hidden_size1, gpu_ind1, NN_type1, L1, S1, n1, zn1, p1, batchnorm_on1,
    ...
    print("Geomatric quantile with penalty")
    if gpu_ind == -1:
        device = 'cpu'
    ...
    G = GNet2(S, hidden_size, L, batchnorm_on, p, n, zn).to(device)
    ...
    for it in range(iteration):
          ...
          #lam11 = lam[0].item()
            ...
            if pen_on == 1:
               #a2 = torch.rand(n0,1).to(device, dtype=torch.float)
                u2 = 2*torch.rand(n0, 1)-1
                u2 = torch.ones(n0, 1)*u2
                u2 = u2.reshape(n0,1).to(device, dtype=torch.float)
        ...
        if (it+1) % 100==0 and verb == 1:
            ...
            .format(float(loss0), LOSS[0], float(pp.item()), lr1, n, 1, train_time,), end='')
            loss0 = 0.0
            pen0 = 0.0
            lam0 = 0.0
            sys.stdout.flush()
    ...
    with torch.no_grad():
      ...
      for i in range(n_lam):
        ...
        lam = lam_cand[i]*torch.ones(N*n_test,1).to(device)
        u = 2*torch.rand(N*n_test, 1)-1
        #u = torch.randn(n_test)
        u = torch.ones(N*n_test, 1)*u
        u = u.reshape(N*n_test,1).to(device, dtype=torch.float)
        
        out_s = G(Xb, u, lam)
        Out[i,:,:] = out_s.reshape(N,n_test)
