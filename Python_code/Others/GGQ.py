...
def GGQ(y, X, Xt, hidden_size1, gpu_ind1, NN_type1, L1, S1, n1, zn1, p1, batchnorm_on1,
    ...
    def Loss(y, yhat, u, yhatt):
        resid = y-yhat
        #out1 = torch.linalg.norm(resid,ord=2,dim=1)+torch.sum(resid*u, dim=1)
        ed = 2*torch.linalg.norm(resid,ord=2,dim=1)-torch.linalg.norm(yhat-yhatt,ord=2,dim=1)
        out = (ed) + torch.sum(resid*u, dim=1)
        #out2 = torch.linalg.norm(y,ord=1,dim=1)+torch.sum(u*y, dim=1)
        return out.mean()
    
    def Loss1(y, yhat, u):
        resid = y-yhat
        out1 = torch.linalg.norm(resid,ord=2,dim=1)+torch.sum(resid*u, dim=1)
        #out1 = 2*torch.linalg.norm(resid,ord=2,dim=1)-torch.linalg.norm(yhat-yhatt,ord=2,dim=1)+torch.sum(resid*u, dim=1)
    ...
    def Open_ball(n00, dim_y, const0):
        direction = np.random.normal(size=(n00, dim_y))
        direction /= np.linalg.norm(direction, axis=1).reshape(n00,1)
        u = (np.random.random(n00) ** (1/dim_y)).reshape(n00,1) * direction * const0
        
        #theta = np.random.rand(n00)*math.pi*2
        #cord = np.ones((n00, dim_y));cord[:,0] = np.cos(theta);cord[:,1] = np.sin(theta)
        #u = np.random.rand(n00, 1)*cord
    ...
    for it in range(iteration):
        ...
        for h in range(K0):
            ...
            u = torch.ones(n0, dim_s)*u
            u = u.to(device, dtype=torch.float)
            
            #energy distance
            u2 = torch.ones(n0, dim_s)*Open_ball(n0, dim_s, const)
            u2 = u2.to(device, dtype=torch.float)
            Out_G2 = G(X0, u2)
            
            Out_G = G(X0, u)
            #print(Out_G.shape)
            #loss = Loss( torch.sum(y0*s_vec, dim=1).reshape(n0,1) - Out_G, alpha)
            #loss = Loss1(y0, Out_G, u)
            loss = Loss(y0, Out_G, u, Out_G2)
    ...
    with torch.no_grad():
      ...
      Out = torch.zeros(N, n_test, dim_s)
      
      u = Open_ball(N*n_test, dim_s, const00)
      u = torch.ones(N*n_test, dim_s)*u
      u = u.to(device, dtype=torch.float)
    
      Out = G(Xb, u).reshape(N, n_test, dim_s)
...
#      for i in range(N):
        ...
        #const0 = 1.0
#        u = Open_ball(N*n_test, dim_s, const)
#        u = torch.ones(N*n_test, dim_s)*u
#        u = u.to(device, dtype=torch.float)
